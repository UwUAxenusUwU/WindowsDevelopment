...
CONST CHAR g_sz_WINDOW_CLASS[] = "Calc_pr311";
...
CONST INT g_i_WINDOW_HEIGHT = g_i_DISPLAY_WIDTH + g_i_START_Y * 2 + (g_i_BUTTON_SIZE + g_i_INTERVAL) * 4;

CONST INT g_i_START_X_BUTTON = g_i_START_X;
CONST INT g_i_START_Y_BUTTON = g_i_START_Y * 2 + g_i_DISPLAY_HEIGHT;
INT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lcCmdLine, INT nCmdShow)
{
		...
		for (int i = 2; i >= 0; i--)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				(
					...
					(HMENU)(IDC_BUTTON_1 + digit++), NULL, NULL
				);
			}
		}
	}
